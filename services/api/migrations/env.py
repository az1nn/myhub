...
from myhub.infrastructure.db import models  # noqa: F401

config = context.config
if config.config_file_name is not None:
    fileConfig(config.config_file_name)

url = os.environ.get("MYHUB_DATABASE_URL")
if not url:
    raise RuntimeError("MYHUB_DATABASE_URL is required for migrations")
